...
private _calculateScore = {
	...
	if (_armorStructural != 0) then {
		...
		if (ETCG_DebugMode) then {diag_log format ["Effective armor calculated: %1", _effectiveArmor];};
	} else {
		if (ETCG_DebugMode) then {diag_log "ArmorStructural is 0 — cannot calculate effectiveArmor";};
	};
...
private _cfgVehicles = if ((count ETCG_exceptions) == 0) then {
    "true" configClasses (configFile >> "CfgVehicles")
} else {
    ETCG_exceptions apply { configFile >> "CfgVehicles" >> _x }
};
...
// ================= LOOP STARTS HERE =================
{
	...
	if ((count ETCG_exceptions) == 0) then {
		if !(_className find ETCG_modName == 0) then {_correctClassname = false;};
	};				  
...
// Notify the user
_outputLog = format ["%1 %2 vehicles have been copied to the clipboard.", count _vehArray, ETCG_modName];
